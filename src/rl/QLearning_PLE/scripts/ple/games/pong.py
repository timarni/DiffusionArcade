...
class Pong(PyGameWrapper):
    ...
    def getGameState(self):
        ...
        state = {
            ...
            # added by damian:
            "total_reward": self.score_sum,
            "total_penalty": self.penalty_sum,
        }
    ...
    def step(self, dt):
        ...
        if self.ball.pos.x <= 0:    # agent loses the point
            ...
            # 'human': version that additionally adds a penalty for moving too much (to avoid oscillations)
            elif self.reward_policy == 'enhanced' or self.reward_policy == 'enhanced_2' or self.reward_policy == 'human':
        ...
        if self.reward_policy == 'human':
            penalty = 1e-06 * abs(self.agentPlayer.vel.y)  # penalize for moving
            self.score_sum -= penalty
        ...
        if abs((self.ball.pos.x - self.ball.radius/2) - (self.agentPlayer.pos.x + self.paddle_width/2)) < epsilon and self.agentPlayer.pos.y - self.paddle_height/2 <= self.ball.pos.y <= self.agentPlayer.pos.y + self.paddle_height/2:
            ...
            if self.reward_policy == 'enhanced_2' or self.reward_policy == 'human':
                self.score_sum += self.rewards["positive"]
