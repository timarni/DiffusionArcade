...
def train_agent(fps=30, display_screen=False):
    ...
    ### PLE set‑up
    game = Pong(width=IMG_W, height=IMG_H, MAX_SCORE=11,
    ...
    print(f"Run stamp of training is {run_stamp}")
    ...
    for ep in tqdm(range(config["n_episodes"])):
        ...
        while not done and step < MAX_STEPS:
            ...
            # Save screen
            if SAVE_SCREEN and (ep+1) in config['save_in_episodes'] and step % config['save_screen_freq'] == 0:
